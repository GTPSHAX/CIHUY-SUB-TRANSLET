class AppLog {
  ...
  fatal(msg, meta) { this.#append("FATAL", 0, msg, meta); }

  /** Level 0 — error: something went wrong but execution can continue. */
  error(msg, meta) { this.#append("ERROR", 0, msg, meta); }

  /** Level 1 — warning: unexpected situation, not yet an error. */
  warn(msg, meta)  { this.#append("WARN",  1, msg, meta); }

  /** Level 2 — normal general-purpose log. */
  log(msg, meta)   { this.#append("LOG",   2, msg, meta); }

  /** Level 3 — success confirmation. */
  success(msg, meta) { this.#append("SUCCESS", 3, msg, meta); }

  /** Level 3 — failure that is handled / expected. */
  fail(msg, meta)    { this.#append("FAIL",    3, msg, meta); }

  /** Level 3 — a service / component is ready. */
  ready(msg, meta)   { this.#append("READY",   3, msg, meta); }

  /** Level 3 — a process is starting. */
  start(msg, meta)   { this.#append("START",   3, msg, meta); }

  /** Level 3 — informational note. */
  info(msg, meta)    { this.#append("INFO",    3, msg, meta); }

  /** Level 4 — debug detail, shown in development. */
  debug(msg, meta)   { this.#append("DEBUG",   4, msg, meta); }

  /** Level 5 — fine-grained trace, shows full execution path. */
  trace(msg, meta)   { this.#append("TRACE",   5, msg, meta); }

  // Utility methods

  /**
   * Returns a copy of all recorded entries, optionally filtered by type label.
   * @param {string} [type] - e.g. 'ERROR', 'DEBUG'
   * @returns {object[]}
   */
  getLogs(type) {
    return type
      ? this.logs.filter((e) => e.type === type.toUpperCase())
      : [...this.logs];
  }
  ...
  withLevel(tempLevel, fn) {
    ...
    try { fn(); } finally { this.level = prev; }
  }
}
